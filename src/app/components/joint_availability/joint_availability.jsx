...
export default function JointAvailability({ course = "Matemáticas" }) {
  ...
  const loadJointAvailability = async () => {
    try {
      ...
      const response = await fetch(`/api/joint-availability?course=${encodeURIComponent(course)}`);
      ...
      if (data.tutors.length === 0) {
        setError(`No se encontraron tutores para la materia "${course}". Verifica que la materia esté escrita correctamente.`);
      }
          ...
          <div className="tutors-list">
            {tutors.length === 0 ? (
              <div className="empty-state">
                ...
                <p>No hay tutores disponibles para {course}</p>
              </div>
            ) : (
              tutors.map((tutor) => {
